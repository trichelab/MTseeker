...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "MTseeker", "tests")); system.time(test_check("MTseeker", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
