MTmouseCircos <- function(variants=NULL, outside=NULL, inside=NULL, outcol=NULL, 
  ...
  circos.clear() 
  data(mtAnno.rCRS)
  if (anno == "rCRS") anno <- mtAnno #.rCRS
  
  ### MAKE THIS INTO A PROPER DATA STRUCTURE
  if (anno = "NC_005089") anno <- gr  # NC_005089
  pfun <- function(x, y) {
    xlim <- CELL_META$xlim
    ylim <- CELL_META$ylim
