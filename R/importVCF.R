...
importVCF <- function(x, verbose = T) {
  ...
  if(is(x, 'character') || is(x, 'list')) {
    vcf_files <- x
    if(verbose) message("Number of vcf files: ", length(vcf_files))
  } else if(is(x, 'data.frame') || is(x, 'dataframe')) {
    if(!("VCF" %in% names(x))) {
      stop("If given input is in data.frame, column \"VCF\" is required")
    } else {
      if("BAM" %in% names(x)) {
        if(verbose) message("Found BAM column in input.")
        bam_files <- as.character(x$BAM)
      }
      vcf_files <- as.character(x$VCF)
    }
  } else {
    stop('vcf must be given in type of either character, list or dataframe')
  }
  ...
  for(i in 1:length(vcf_files)) {
    ...
    if(!is.null(bam_files)) { bam_file <- bam_files[i] }
...
vcf2mvrl <- function(vcf_filename, bam_filename = NULL, verbose = T) {
  ...
  result <- tryCatch({
    ...
    if(verbose) message("Found ", ncol(collVcfs), " sample(s) in VCF file ... ")
...
calc_cov <- function(ranges_obj, bam_file, chr, verbose=T) {
  if(verbose) message("Calculation coverage in region of chromosome M .. ")
...
.calcVCF_strelka <- function(collVCF) {
  cnt_dt <- cbind(geno(collVCF)$AU[,1], 
                  geno(collVCF)$CU[,1],
                  geno(collVCF)$GU[,1],
                  geno(collVCF)$TU[,1]
            )
  
  ref_idx <- .alleles2idx(as.character(collVCF@fixed$REF))
  alt_idx <- .alleles2idx(as.character(collVCF@fixed$REF))

  return(calcVAF_strelkaC(ref_idx, alt_idx, cnt_dt))  
}



.alleles2idx <- function(alleles) {
  alleles[alleles == 'A'] <- 0
  alleles[alleles == 'C'] <- 1
  alleles[alleles == 'G'] <- 2
  alleles[alleles == 'T'] <- 3
...
Rcpp::cppFunction('
NumericVector calcVAF_strelkaC(NumericVector ref_idx, NumericVector alt_idx, NumericMatrix cnt_dt) 
{
  int len = ref_idx.size();
  NumericVector res(len);

  for(int i = 0;i < len;i++) {
    res[i] = cnt_dt(i, alt_idx[i]) / (cnt_dt(i, alt_idx[i]) + cnt_dt(i, ref_idx[i]));
  }
